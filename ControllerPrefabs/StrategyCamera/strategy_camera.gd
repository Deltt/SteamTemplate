...
func _physics_process(delta: float) -> void:
	...
	camera.position.z = lerpf(camera.position.z, current_distance, min(lerp * delta, 1))
	...
	lerped_movement = lerped_movement.lerp(key_input_relative, min(lerp * delta, 1))
